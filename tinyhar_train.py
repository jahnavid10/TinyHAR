...
def scaling(x):
    x = np.clip(x, 0.0, 94.0)
    return 2.0 * (x / 94.0) - 1.0
...
def make_windows(X, y, win, overlap):
    ...
    while i + win <= n:
        ...
        # ignore 'null' in majority vote
        nnz = seg_y[seg_y != 0]
        if nnz.size > 0:
            maj, maj_cnt = Counter(nnz).most_common(1)[0]
            # require >=60% of the whole window to be one non-null class
            if maj_cnt / float(len(seg_y)) >= 0.40:
                label_str = next((k for k, v in LABEL_DICT.items() if v == maj), None)
                if label_str and label_str != 'null':
                    # ZERO-MEAN the window per channel (preserves temporal shape)
                    seg_X = seg_X - seg_X.mean(axis=0, keepdims=True)
                    wins.append(seg_X)
    ...
    if not wins:
        return np.empty((0, win, X.shape[1]), np.float32), np.empty((0,), np.int64)
    return np.stack(wins).astype(np.float32), np.array(labels, np.int64)
...
def main():
    ...
    for test_sid in SUBJECTS:
        ...
        inv_freq_w = (class_counts.sum() / (class_counts + 1e-8))
        inv_freq_w = inv_freq_w / inv_freq_w.mean()
        class_w = torch.tensor(inv_freq_w, dtype=torch.float32, device=DEVICE)
